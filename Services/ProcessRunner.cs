...
{
    public static async Task<ProcessResult> RunAsync(string file,IEnumerable<string> args,int timeout=15000,IDictionary<string,string?>? env=null,string? workingDirectory=null)
    {
        var info=new ProcessStartInfo(file){UseShellExecute=false,CreateNoWindow=true,RedirectStandardOutput=true,RedirectStandardError=true,WindowStyle=ProcessWindowStyle.Hidden,WorkingDirectory=workingDirectory??""}; foreach(var arg in args)info.ArgumentList.Add(arg); if(env is not null)foreach(var x in env)info.Environment[x.Key]=x.Value;
