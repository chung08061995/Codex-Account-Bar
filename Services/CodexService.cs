...
public sealed class CodexService
{
    ...
    public async Task<string> LoginIsolatedAsync()
    {
        ...
        try{var args=command.IsCmd?new[]{"/d","/c",command.Path,"login","-c","cli_auth_credentials_store=\"file\""}:new[]{"login","-c","cli_auth_credentials_store=\"file\""};var r=await ProcessRunner.RunAsync(command.IsCmd?"cmd.exe":command.Path,args,600000,new Dictionary<string,string?>{{"CODEX_HOME",home}});var path=Path.Combine(home,"auth.json");if(r.ExitCode!=0||!File.Exists(path))throw new InvalidOperationException("Codex sign-in failed or was cancelled. "+r.Error.Trim());return await File.ReadAllTextAsync(path);}finally{try{Directory.Delete(home,true);}catch{}}
    }
